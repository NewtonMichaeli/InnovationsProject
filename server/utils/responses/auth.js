...
// user : success
const loggedInSuccessfully = (res, token) => res.status(200).json({status: true, msg: 'Successfully logged in'}).cookie(AUTH_TOKEN, token).end()
const userCreatedSuccessfully = (res, token) => res.status(200).json({status: true, msg: 'User created successfully'}).cookie(AUTH_TOKEN, token).end()
const userUpdatedSuccessfully = (res, new_data, new_token) => 
    res.status(200).json({status: true, msg: 'User updated successfully', data: {new_data}}).cookie(AUTH_TOKEN, new_token).end()
